	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(boUser.isLogin(request, response)){//neu ton tai session
			...
			if (tenDanhMuc != null) {// tim kiem
				...
				int total = boDanhMuc.getTotalSearchAdmin(tenDanhMuc);
				int row_count = boPhanTrang.getRowCountAdmin();
				request.setAttribute("sotrang", boPhanTrang.getSoTrang(total,row_count));
				request.setAttribute("current_page", current_page);
					// neu load trang
				ArrayList<DanhMuc> listDanhMuc = boDanhMuc.searchDanhMucAdmin(tenDanhMuc, boPhanTrang.getOffset(current_page, row_count), row_count);
